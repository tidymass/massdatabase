...
convert2metid <-
  ...
  for (i in seq_along(raw_names)) {
    idx <- which(colnames(ms1_info) %in% raw_names[[i]])
    if (length(idx) > 0) {
      colnames(ms1_info)[match(colnames(ms1_info)[idx], 
                               colnames(ms1_info))] <- final_names[i]
    }
    else {
      ms1_info <- data.frame(ms1_info, new = NA)
      colnames(ms1_info)[ncol(ms1_info)] <- final_names[i]
    }
  }
  ...
  ms1_info <- ms1_info %>% 
               ...
               dplyr::select(c(check_names, Submitter, everything()))
  
  ## add Collision_Energy Parameter
  if (any(colnames(ms1_info) == "Collision_energy")) {
    ms1_info <- ms1_info %>% dplyr::rename(CE = Collision_energy)
  }
  else {
    ms1_info <- ms1_info %>% dplyr::mutate(CE = NA)
  }
  
  ## Add KEGG.ID Info
  if(any(colnames(ms1_info) != "KEGG.ID")){
    ms1_info <- ms1_info %>% dplyr::mutate(KEGG.ID = NA)
  }
  ## add HMDB.ID Info
  if(any(colnames(ms1_info) != "HMDB.ID")){
    ms1_info <- ms1_info %>% dplyr::mutate(HMDB.ID = NA)
  }
  ### add CAS.ID Info
  if(any(colnames(ms1_info) != "CAS.ID")){
    ms1_info <- ms1_info %>% dplyr::mutate(CAS.ID = NA)
  }
  ## Add RT Info
  if(any(colnames(ms1_info) != "RT")){
    ms1_info <- ms1_info %>% dplyr::mutate(RT = NA)
  }
  ## add Adduct Info
  ...
  ms1_info <- ms1_info %>% 
               dplyr::mutate(Polarity = case_when(Polarity == "POSITIVE" ~ "Positive", 
  ...
  if (all(is.na(ms1_info$mz))) {
    ...
    ms1_info$mz <- purrr::map(seq_len(nrow(ms1_info)), .f = function(i) {
      x <- ms1_info$Precursor_mz[i]
      y <- ms1_info$Adduct[i]
      z <- ms1_info$Formula[i]
      mz <- tryCatch(Rdisop::getMass(Rdisop::getMolecule(z)), 
                     error = function(e) {
                       NA
                     })
      if (is.na(mz)) {
        y <- y %>% stringr::str_replace("\\]\\+", "") %>% 
          stringr::str_replace("\\]\\-", "") %>% stringr::str_replace("\\[", "") %>% 
          stringr::str_replace("\\]", "")
        mz <- masstools::convert_precursor_mz2accurate_mass(precursor_mz = x, adduct = y)
      }
      mz
    }) %>% unlist() %>% as.numeric()
    message("Done.")
    ...
    message("calculate mz is ongoing !!!")
  }
  
  rownames(ms1_info) <- NULL
  remove_idx <- which(is.na(ms1_info$mz))
  if (length(remove_idx) > 0) {
    ms1_info <- ms1_info[-remove_idx, ]
    spectra_data <- spectra_data[-remove_idx]
  }
  ...
  write.csv(ms1_info, "./convert2metid2_ms1_info.csv")
  ms1_info2 <- ms1_info %>% plyr::dlply(.variables = "Lab.ID") %>% 
    purrr::map(function(y) {
      if (sum(is.na(y$CE)) > 0) {
        y$CE[is.na(y$CE)] <- paste("Unknown", 1:length(y$CE[is.na(y$CE)]), 
                                   sep = "_")
      }
      y
  ...
  spectra_data2 <- seq_along(spectra_data) %>% purrr::map(function(i) {
    if (i %in% progresser$idx) {
      message(progresser$progresser[which(i == progresser$idx)], 
              " ", appendLF = FALSE)
    }
  ...
  readr::write_csv(x = ms1_info2, file = file.path(temp_file, 
                                                   "ms1_info2.csv"))
  
  in_house_ms2 <- metid::construct_database(path = temp_file, 
                                            version = as.character(Sys.Date()), metabolite.info.name = "ms1_info2.csv", 
                                            source = Submitter, link = "https://metid.tidymass.org/", 
                                            creater = Submitter, email = "shenxt1990@outlook.com", 
                                            rt = FALSE, threads = threads)
  unlink(file.path(temp_file, "ms1_info2.csv"))
  unlink(temp_file)
  in_house_ms2@spectra.data$Spectra.positive <- spectra_data_pos
  in_house_ms2@spectra.data$Spectra.negative <- spectra_data_neg
